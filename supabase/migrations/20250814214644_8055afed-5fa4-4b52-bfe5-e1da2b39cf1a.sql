...
-- Update the original table to only allow admin access
-- Regular users should use the function instead
DROP POLICY "Only admins can access full invite data" ON public.invites;

CREATE POLICY "Only admins can access invite table directly" 
ON public.invites 
FOR SELECT 
USING (EXISTS (
